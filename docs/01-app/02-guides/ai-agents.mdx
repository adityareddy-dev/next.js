...
[`agentRules`](/docs/app/api-reference/config/next-config-js/agentRules) keeps AI coding agents aligned with your installed Next.js version by directing them to the bundled docs. Next.js writes the following managed instructions to `AGENTS.md` or `CLAUDE.md` at the project root:
...
Next.js updates only the content between the agent rules markers and preserves the rest of the file. The option is enabled by default on Next.js 16.3 or later. Set `agentRules` to `false` to remove the managed block the next time you run `next dev`.
...
Agent feedback lets AI coding agents keep track of de-identified Next.js issues while they work and prepare them for your review at the end of the task. Each report opens in a review form where you can edit or remove details before choosing whether to send it. Agent feedback is separate from [Next.js Telemetry](/telemetry).
