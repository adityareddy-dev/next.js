...
On Next.js 16.3 or later, [`agentRules`](/docs/app/api-reference/config/next-config-js/agentRules) is enabled by default. When `next dev` detects an AI coding agent, Next.js creates or updates managed documentation instructions in `AGENTS.md` or `CLAUDE.md` at the project root:
...
Next.js updates only the content between the agent rules markers and preserves the rest of the file. Set `agentRules` to `false` to stop creating or updating the block. An existing agent rules block remains in the file.

[Benchmark results on nextjs.org/evals](https://nextjs.org/evals) show that agents perform better when they read the bundled docs. See the [`agentRules` API reference](/docs/app/api-reference/config/next-config-js/agentRules) to disable generated instructions.

### Agent feedback

Experimental [`agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) is disabled by default. When enabled and `next dev` detects an AI coding agent, Next.js creates or updates managed feedback instructions in `AGENTS.md` or `CLAUDE.md` at the project root. Agent feedback is separate from [Next.js Telemetry](/telemetry).

The feedback instructions use a separate managed block:
...
Next.js updates only the content between the agent feedback markers and preserves the rest of the file. Disabling or removing `experimental.agentFeedback` removes the existing feedback block the next time you run `next dev`.

When an agent prepares a report, it opens in a review form. Nothing is sent until you select **Send feedback**. You can edit or remove details, or close the form without sending.
