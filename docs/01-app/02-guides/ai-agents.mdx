...
Next.js updates only the content between the agent rules markers and preserves the rest of the file. Set `agentRules` to `false` to remove the managed block the next time you run `next dev`.
