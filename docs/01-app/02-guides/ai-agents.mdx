...
The managed block tells the agent to retain potential Next.js friction during the task. At the final stopping point, the command checks whether reporting is available and returns the version-matched instructions for deciding whether the friction qualifies and, if so, structuring one report. The command runs at most once per task, including when a Next.js Skill also applies.

Nothing is sent when a report opens. You can edit or remove details, close the form without sending, or send the reviewed report.
