...
The managed block tells the agent which friction to retain during the task. At a stopping point, the command checks whether reporting is available and returns the version-matched instructions for selecting, structuring, and reviewing a report.
