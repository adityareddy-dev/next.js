...
The `agentRules` option controls whether `next dev` creates and updates the managed agent instructions above. It is enabled by default. [Benchmark results on nextjs.org/evals](https://nextjs.org/evals) show that agents perform better when they read the bundled docs.

Set `agentRules` to `false` to disable generation:
