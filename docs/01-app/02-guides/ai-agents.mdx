...
On Next.js 16.3 or later, the [`agentRules`](/docs/app/api-reference/config/next-config-js/agentRules) option is enabled by default. When you run `next dev` with an AI coding agent, Next.js creates or updates managed instructions in `AGENTS.md` or `CLAUDE.md` at the project root:
...
The experimental [`agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) option lets detected AI coding agents prepare Next.js issue reports for your review. It is off by default and separate from [Next.js Telemetry](/telemetry).

When enabled and an AI coding agent is detected, `next dev` adds a separate managed block to `AGENTS.md` or `CLAUDE.md`:
...
Next.js preserves content outside the managed block. When an agent prepares a report, it opens in a review form. Nothing is sent until you select **Send feedback**. You can edit or remove details, or close the form without sending.
