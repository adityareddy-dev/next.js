...
The `agentRules` option controls whether `next dev` creates and updates version-matched instructions for detected AI coding agents. The option is enabled by default.
...
Setting `agentRules` to `false` prevents `next dev` from creating or updating the managed block. It does not remove a block that already exists.
...
`agentRules` works independently from the experimental [`agentFeedback`](/docs/app/api-reference/config/next-config-js/agentFeedback) option. Changing one option does not affect the other.
