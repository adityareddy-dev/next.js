...
Set `experimental.agentFeedback` to `false` or remove the option to stop `next dev` from creating or updating the managed block. The next `next dev` run removes the existing agent feedback block and preserves all other content. A file that contained only the managed block is deleted; any other `AGENTS.md` or `CLAUDE.md` content is kept.
